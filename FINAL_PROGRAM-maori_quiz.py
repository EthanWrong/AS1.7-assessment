"""This is the final version of the Maori Quiz program that I developed for my
Programming (1.7 AS91883) and Iterative Processes (1.8 AS91884) standards for
NCEA Level 1 Computer Science, to be submitted by Thursday 3 June 2022.
by Ethan Wong

All code meets the PEP8 requirements.
THIS CODE MUST GO ALONGSIDE:
- library_v2.py
- text_decorator.py
(These files are also coded by me, Ethan Wong)
"""
import library_v2
import random
import time
...
# choice interpreter that checks choice is valid, and does action
# accordingly for collecting words
def choice_selection(master):
    collections = []
    choice = str.strip(input(f"{GI_TXT}From what category of words would "
                             f"you like to learn? \n{GI_TXT} Enter '?' for "
                             f"category options >>> ").lower())

    while True:
        if choice == "all":
            master.clear()
            collections.clear()
            # goes through and adds all the possible collections to master-list
        ...
        if not collections:  # if nothing has been chosen
            choice = str.strip(input(f"{GI_TXT}From what collection of words "
                                     f"would you like to learn? >>> ").lower())

        else:  # if something has already been chosen
            choice = str.strip(input(f"{GI_TXT}If you would like to add "
                                     f"another collection, do so.\n{GI_TXT} "
                                     f"Otherwise type 'x' >>> ").lower())

    print(f"{GI_TXT}<< Word selection COMPLETE >> ")
    print()
...
def abcd_checker(question, a, b, c=None, d=None):
    while True:
        ...
        answer = str.strip(input(f"{GI_TXT}{question.capitalize()} "
                                 f">>> ").lower())
...
# function that asks the user for an answer and tests to see if it is correct
def test_user(question, answers, question_number, language="undefined"):
    ...
    user_answer = str.strip(input(question_text).lower())
...
# function to remember mistakes for later on in Review Mistakes
...
# function that selects a question and tests the user
def play_round(chosen_language, master, question_number):
    # code that chooses a word-set, defines the language, then gets the
    # question and answers from that word set
...
# function that returns a list i.e [minutes, seconds]
...
def instructions():
    print()
    print(f"{GI_TXT}This word-translation game will help you learn some Maori "
          f"words.\n")
    print(f"{GI_TXT}First you will select which categories of words you "
          f"wish to learn. You can\n{GI_TXT} choose "
          f"{', '.join(library_v2.collections)},\n{GI_TXT} or some of them, "
          f"or all of them.\n")
    print(f"{GI_TXT}Next, you will choose which language you wish to answer "
          f"in: Maori, English,\n{GI_TXT} or Both\n")
    print(f"{GI_TXT}Then the game will start, and you will be asked to "
          f"translate words into\n{GI_TXT} your chosen language until you have"
          f" either translated them all, or you\n{GI_TXT} stop the game by "
          f"entering 'x'\n")
    print(f"{GI_TXT}Every time you get a question wrong, it will be recorded "
          f"as a mistake and\n{GI_TXT} you will be given two more of that same"
          f" question (practice makes\n{GI_TXT} perfect). To help you, the "
          f"game will also give you all the possible\n{GI_TXT} answers\n")
    print(f"{GI_TXT}Many words have multiple different answers for each "
          f"language:\n{GI_TXT} In Maori, there are usually a couple different"
          f" translations which are\n{GI_TXT}  valid, and answering without "
          f"macrons is also valid\n {GI_TXT}In English, you can answer with "
          f"the first three letters of the days and\n{GI_TXT}  months, and "
          f"just the numeral for numbers\n{GI_TXT} (Remember, making mistakes"
          f" is how you learn, so make sure you check all\n{GI_TXT} the other "
          f"possible answers that work if you get one wrong)\n")
    print(f"{GI_TXT}At the end of the game you will see your stats such as "
          f"questions\n{GI_TXT} right/wrong and how fast you were at answering"
          f" them. You will also have\n{GI_TXT} the option to review your "
          f"mistakes and see all the questions you got\n{GI_TXT} wrong.\n")
    print(f"{GI_TXT}Finally, you will be given your 'Word to Revise'--this is "
          f"the word you got\n{GI_TXT} wrong the most, so practice it!\n")
    print()
...
# function that allows the user to see all their mistakes
def review_mistakes(mistakes):
    ...
    print()
    # just waits for the user to hit enter
    input(f"{GI_TXT}Press enter to get your Word to Revise >>> ")
    print()
...
# function that displays user's stats
